...
def build_prompt(registry: dict[int, dict], target_week: str) -> str:
    ...
    instructions = (
        ...
        "  - SIGLES & JARGON : aucun acronyme ni terme spécialisé opaque. Développe tout\n"
        "    SIGLE à son 1er emploi (ex. « l'IMREDD, l'institut de l'Université Côte\n"
        "    d'Azur dédié à la ville durable » ; « le MIN — marché d'intérêt national »).\n"
        "    TRADUIS/EXPLIQUE le jargon, en particulier les termes italiens : « OPAS »\n"
        "    → « offre publique d'achat et d'échange (OPA) » ; écris les noms en entier\n"
        "    au 1er emploi (« Monte dei Paschi di Siena (Mps) », « Banca Mps »). Public\n"
        "    business mais qui ne maîtrise pas forcément le jargon financier/admin :\n"
        "    reste précis ET compréhensible, sans condescendance.\n\n"
